...
namespace pbd {
	CNHTetraVolume::CNHTetraVolume()
		: ids{0,0,0,0}
		, hydrostatic_compliance(1.f)
		, deviatoric_compliance(1.f)
		, inv_rest(1.f)
	{}
	CNHTetraVolume::CNHTetraVolume(const std::array<int32_t, 4> _ids, const glm::mat3& _irest, float hydro, float devia)
		: ids(_ids)
		, inv_rest(_irest)
	...
	void CNHTetraVolume::eval(Engine& engine, float rdt2) const {
		std::array<Particle*, 4> p;
	...
	void CNHTetraVolume::remap(int32_t offset) {
		for (int32_t& id : ids) {
			id += offset;
		}
	}
	void CNHTetraVolume::transform(const Transform3& form) {
